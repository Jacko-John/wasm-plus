...
void parse_table_type(Module *m, u32 *pos) {
    ...
    m->table.elem_type = read_LEB128_unsigned(m->bytes, pos, 7);
    ASSERT(m->table.elem_type == TYPE_REF_FUNC, "Table elem_type 0x%x unsupported\n", m->table.elem_type)
    ...
    if (flags & 0x1) {
        ...
        m->table.max_size = (u32) fmin(0x10000, tsize);
    } else {
